...
## 42、preconnect、dns-prefetch 和 preload 有什么区别
`dns-prefetch`、`preconnect` 和 `preload` 都是资源提示能力，用于提前完成部分网络或资源加载工作，但它们优化的阶段不同。
...
`preconnect` 会提前建立连接，通常包括 DNS、TCP 和 TLS：

```html
<link rel="preconnect" href="https://cdn.example.com" />
```

适合关键跨域资源，例如字体、首屏接口或 CDN。

### preload

`preload` 用于提前加载当前页面很快会用到的关键资源：
...
- 只想提前解析域名：`dns-prefetch`。
- 关键第三方域名连接成本高：`preconnect`。
- 当前页面关键资源需要尽早下载：`preload`。

### 注意事项

- 不要 preload 太多资源，否则会抢占关键带宽。
- 字体 preload 通常需要 `crossorigin`。
- preconnect 会占用连接资源，不适合大量域名。
- 资源提示要结合瀑布图验证效果。
:::

## 43、HTML 表格语义化要注意什么
HTML 表格适合展示二维数据。语义化表格可以让浏览器、搜索引擎和辅助技术更好地理解表头、数据单元格和表格说明之间的关系。
