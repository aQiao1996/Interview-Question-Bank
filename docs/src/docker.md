...
## 23、Docker 容器安全加固有哪些手段
Docker 容器安全加固的目标是缩小攻击面、限制权限、隔离敏感资源并提高检测能力。它通常从镜像、运行时、挂载和网络几方面一起做。
...
## 24、Docker 数据卷如何备份和恢复
Docker 数据卷常用于持久化数据库和业务数据。备份恢复要考虑 Volume 来源、数据一致性和恢复验证，不能只把文件拷走就算完成。

::: details 详情
### 备份思路

常见做法是把 Volume 内容临时挂载到一个备份容器，再打包导出：
...
- 备份要定期演练恢复。
- 不同 Volume 的权限和 UID/GID 可能不同。
- 备份文件要加密并控制访问权限。
- 重要数据要保存到异地或独立存储。
:::

## 25、Docker 和 Kubernetes 的关系和分工是什么
Docker 和 Kubernetes 不是同一层的问题。Docker 主要解决镜像构建和容器运行，Kubernetes 主要解决容器编排、调度和高可用管理。
...
```txt
Docker 负责做镜像和单容器运行
Kubernetes 负责编排和管理多个容器实例
```

很多团队会同时使用两者：Docker 构建镜像，Kubernetes 部署和管理运行实例。

### 注意事项

- 不要把 Docker 当成完整的集群编排方案。
- Kubernetes 也不替代镜像构建流程。
