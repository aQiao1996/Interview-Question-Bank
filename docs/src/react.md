...
## 49、React 如何避免不必要的重新渲染
React 中避免不必要渲染的核心是减少状态影响范围、保持 props 稳定、合理拆分组件，并用性能工具确认真正的瓶颈。
...
- 列表使用虚拟滚动。
- Context 拆分，避免一个大 Context 变化影响所有消费者。

### props 稳定性

如果每次渲染都创建新对象或新函数，子组件即使用了 `memo` 也可能重新渲染：

```jsx
<Child options={{ pageSize: 20 }} />
```

可以把稳定配置移到组件外，或用 `useMemo` 缓存。

### 注意事项

- `memo`、`useMemo`、`useCallback` 本身也有成本。
- 不要为了减少一次轻量渲染让代码复杂很多。
- 列表性能问题通常优先考虑虚拟滚动，而不是只靠 memo。
- 服务端状态缓存可以交给 React Query、SWR 等库处理。
:::
