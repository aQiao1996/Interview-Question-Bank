...
## 28、vue2 为什么更新 DOM 是异步的
Vue2 中数据变化后不会立刻同步更新 DOM，而是把相关 watcher 放入队列，在本轮事件循环结束前统一刷新。

::: details 详情
### 为什么要异步更新

如果每次修改数据都立即更新 DOM，连续多次修改会触发多次重复渲染：
...
Vue2 会把这些变化合并到同一次更新队列中，减少重复计算和 DOM 操作。
...
1. 数据 setter 被触发。
2. 依赖收集到的 watcher 被通知更新。
3. watcher 不会立即执行，而是进入队列。
4. 同一个 watcher 会去重。
5. 在 `nextTick` 中统一刷新队列。
6. 执行 render watcher，生成新的虚拟 DOM 并 patch 到页面。

### nextTick 的作用

`this.$nextTick()` 可以在 DOM 更新完成后执行回调：
...
如果在修改数据后立刻读取 DOM，可能拿到的还是旧内容。

### 底层异步策略

Vue2 会优先使用微任务，例如 `Promise.then`、`MutationObserver`，不支持时再降级到 `setImmediate` 或 `setTimeout`。

### 注意事项

- `nextTick` 不是让数据更新，而是等待 DOM 更新完成。
...
## 35、vue2 中 v-cloak 有什么作用
`v-cloak` 用于在 Vue 实例编译完成前隐藏未编译的模板内容，避免用户看到 `{{ message }}` 这类插值表达式闪烁。
...
Vue 完成模板编译后，会自动移除元素上的 `v-cloak` 属性，内容再显示出来。

### 解决什么问题

在直接通过 CDN 引入 Vue，或者页面脚本加载较慢时，浏览器可能先渲染原始模板：

```html
{{ message }}
```

`v-cloak` 可以在 Vue 接管页面前先隐藏这部分内容。
...
- `v-cloak` 是编译前隐藏，编译后自动移除。
- `v-if` 是运行时条件渲染，依赖 Vue 响应式状态。
- `v-cloak` 需要配合 CSS 才有效。

### 注意事项

- 工程化项目通常通过构建产物挂载，较少需要 `v-cloak`。
- 如果页面一直隐藏，要检查 Vue 是否初始化失败或挂载节点是否正确。
- `v-cloak` 只能解决模板闪烁，不能替代加载态设计。
:::

## 36、vue2 中 scoped CSS 的原理是什么
`scoped` CSS 用于让单文件组件中的样式尽量只作用于当前组件，Vue 编译时会给元素和选择器添加特殊属性来实现样式作用域。
...
```html
<div class="title" data-v-xxxx>标题</div>
```
...
`scoped` 不是 Shadow DOM，而是通过属性选择器模拟局部作用域。

因此它仍然遵循普通 CSS 的级联、继承和优先级规则。

### 父子组件样式影响

父组件的 scoped 样式通常不会直接影响子组件内部元素，但子组件根元素可能同时带有父组件和子组件的作用域属性，所以父组件可以影响子组件根节点样式。

### 深度选择器

如果确实需要修改子组件内部样式，可以使用深度选择器：
...
在不同构建工具中也可能写作 `>>>` 或 `::v-deep`。

### 注意事项

- scoped 不能替代合理的组件样式设计。
- 深度选择器会打破封装，应谨慎使用。
- 属性选择器会让生成后的选择器更长，复杂选择器要控制性能和可读性。
- 全局样式、主题样式和组件局部样式要分层管理，避免互相覆盖。
:::

## 37、vue2 中自定义组件如何支持 v-model
Vue2 中自定义组件的 `v-model` 本质上是 `value` prop 和 `input` 事件的语法糖，组件需要接收 `value` 并在值变化时触发 `input`。
...
Vue2 可以通过 `model` 选项修改默认 prop 和事件名：
...
- `v-model` 默认用于表达组件的主值。
- `.sync` 常用于同步多个 prop。
- Vue2 一个组件默认只能有一个 `v-model`。

### 注意事项

- 子组件不要直接修改 prop，要通过事件通知父组件更新。
