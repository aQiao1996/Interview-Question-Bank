...
## 20、解释事件代理和事件委托的区别
- 日常前端语境里，`事件代理` 和 `事件委托` 通常可以视为同一个概念，基本都指：利用事件冒泡，把多个子元素的事件统一绑定到父元素上处理。
- 更常见、更标准的说法是 `事件委托（event delegation）`，它的核心价值是减少事件监听器数量，并且方便处理动态新增的子元素。
::: details 详情
|方面|说明|
|---|---|
|定义|把事件监听绑定在父元素上，由父元素统一处理子元素触发的事件|
|原理|利用事件冒泡，通过 `event.target` 或 `event.currentTarget` 判断实际触发源|
|优点|减少监听器数量、提升可维护性、适合动态列表或批量子节点|
|典型场景|列表点击、表格操作、菜单项交互、动态渲染节点事件处理|
|示例|把点击事件绑定到父 `ul`，统一处理所有 `li` 的点击|
:::
