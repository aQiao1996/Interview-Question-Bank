...
## 28、浏览器事件循环是如何工作的
浏览器事件循环用于协调 JavaScript 执行、任务队列、微任务、渲染和用户交互。它决定了异步回调什么时候执行。
...
微任务会在当前宏任务结束后尽快执行，并且通常会在渲染前清空。

### 注意事项

- 微任务过多会阻塞渲染。
- 长任务会导致页面卡顿和交互延迟。
- `setTimeout(fn, 0)` 不代表立刻执行。
- 理解事件循环有助于分析异步顺序和性能问题。
:::

## 29、哪些资源会阻塞浏览器渲染
浏览器渲染可能被 CSS、同步 JavaScript、字体和主线程长任务影响。理解阻塞来源有助于优化首屏速度和交互体验。
...
浏览器通常需要等待关键 CSS 下载和解析完成后，才能正确绘制页面。
...
图片通常不阻塞 DOM 解析，但大图会影响 LCP 和带宽竞争。

需要合理设置图片尺寸、压缩和懒加载。

### 注意事项

- `defer` 脚本按顺序执行，且在 DOM 解析后执行。
- `async` 脚本下载完成后立即执行，顺序不固定。
- 首屏资源优先级会影响用户感知速度。
- 性能分析要结合 DevTools、Performance 和 Lighthouse。
:::

## 30、IndexedDB 适合存储什么数据
IndexedDB 是浏览器提供的客户端结构化存储能力，适合存储较大量、需要索引查询或离线使用的数据。它比 localStorage 更适合复杂数据场景。
...
## 31、WebSocket 为什么需要心跳和重连
WebSocket 是长连接协议，但网络切换、代理超时、服务重启和浏览器休眠都可能导致连接异常。心跳用于发现连接是否仍然可用，重连用于恢复通信。

::: details 详情
### 心跳作用

心跳通常是客户端和服务端定期发送轻量消息。
