...
## 32、前端项目如何做依赖安全治理
依赖安全治理是指对 npm 依赖的漏洞、许可证、来源和版本变更进行持续管理，降低供应链风险。

::: details 详情
### 为什么需要治理

前端项目通常依赖大量第三方包，风险可能来自：
...
- 提交 lockfile，锁定依赖树。
- 使用 `npm audit`、`pnpm audit` 或安全平台扫描漏洞。
- 开启 Dependabot、Renovate 等自动升级工具。
- 对高风险依赖做人工评审。
- 控制私有源和 npm registry 配置。
- 在 CI 中加入依赖安全检查。
...
可以把高危漏洞作为阻断条件，低危漏洞进入定期治理列表。

### 版本升级策略

- patch 升级通常风险较小，可以自动化。
- minor 升级需要关注变更说明。
- major 升级应单独评估兼容性。
- 安全漏洞修复可以走紧急升级流程。
...
- 不要盲目升级所有依赖，升级本身也可能引入风险。
- 漏洞扫描要结合实际使用路径判断影响。
- 对核心构建链路依赖要更谨慎，例如打包器、转译器、脚手架。
- 依赖安全治理应持续进行，而不是上线前临时处理。
:::

## 33、前端构建产物如何设计版本回滚
构建产物版本回滚是指线上版本出现问题时，能够快速切换回上一个稳定版本，降低故障影响范围。
...
如果这些内容没有统一版本管理，回滚时可能出现 HTML 和静态资源不匹配。
...
```txt
releaseId = commit hash + build number
```

并把产物上传到版本目录：

```txt
/releases/20260527-abc123/
```
...
```txt
current -> /releases/20260527-abc123/
```

回滚时只切换 `current` 指针或发布配置，而不是重新构建旧代码。

### 需要配套什么

- 保留最近多个版本产物。
- Source Map 和 releaseId 绑定。
- 监控告警能关联版本。
- CDN 缓存刷新策略清晰。
- 回滚后能验证核心页面可访问。
...
- 回滚最好使用已验证产物，不要临时重新打包。
- HTML 缓存策略要谨慎，否则用户可能仍访问异常入口。
- 接口、数据库或配置变更如果不兼容，单纯回滚前端可能不够。
- 发布系统应支持一键回滚和回滚审计记录。
:::

## 34、前端如何分析和优化构建产物体积
构建产物体积分析用于找出首屏包过大、重复依赖、无效代码和拆包不合理等问题，从而改善加载性能。
...
```txt
查看最大 chunk -> 定位模块来源 -> 判断是否首屏必要 -> 拆分或替换依赖
```

不要只看原始体积，也要看压缩后体积和实际加载时机。
...
```txt
main chunk gzip > 250KB 时阻断合并或发出告警
```
...
## 42、Turborepo 在 Monorepo 中有什么作用
Turborepo 是 Monorepo 场景下的任务编排和缓存工具，主要用于提升构建、测试、lint 等脚本的执行效率。
...
如果每次改动都全量构建所有包，会浪费大量时间。Turborepo 可以根据依赖关系和文件变更，只执行必要任务。

### 核心能力

- 任务编排：按包之间的依赖顺序执行脚本。
- 本地缓存：相同输入下复用上一次输出。
- 远程缓存：团队成员或 CI 之间共享构建缓存。
- 增量执行：只处理受影响的包。
- 并行执行：互不依赖的任务可以并行跑。
...
`^build` 表示先构建当前包依赖的其他包。

### 面试回答要点

- Turborepo 不是包管理器，通常和 pnpm workspace、npm workspace 配合使用。
- 它关注任务执行和缓存，不负责发布版本。
- 缓存是否命中取决于输入文件、环境变量、命令和输出声明。
- CI 中接入远程缓存可以明显缩短构建时间。

### 注意事项

- `outputs` 配置错误会导致缓存不准确。
- 依赖关系要通过 workspace 和 package.json 正确声明。
- 不要缓存包含随机数、时间戳、机器路径的不可复现输出。
- 敏感环境变量要谨慎进入缓存输入。
:::

## 43、前端 CI 缓存应该如何设计
前端 CI 缓存的目标是减少重复安装依赖、重复构建和重复测试的时间，但缓存必须可控、可失效，不能影响构建结果的正确性。
...
- 包管理器下载缓存。
- `node_modules`。
- 构建工具缓存，例如 Vite、Webpack、Babel、Turborepo。
- 测试缓存。
- 浏览器二进制文件，例如 Playwright。
...
```txt
os + node版本 + 包管理器版本 + lockfile hash
```

例如 lockfile 变化时，依赖缓存应该自动失效。

### pnpm 场景

pnpm 更适合缓存 store，而不是直接缓存完整 `node_modules`：

```txt
~/.pnpm-store
```

这样可以减少缓存体积，也更符合 pnpm 的硬链接机制。
...
如果环境变量会影响构建结果，也要进入缓存 key 或工具的输入声明。

### 注意事项

- 缓存命中不代表构建一定正确，仍要跑必要校验。
- 不要缓存包含密钥、token、用户目录绝对路径的内容。
- 缓存要设置失效策略，避免长期污染。
- CI 异常时可以提供跳过缓存或清理缓存的手段。
:::
