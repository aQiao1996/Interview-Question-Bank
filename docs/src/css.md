...
## 40、CSS 中层叠上下文是什么
层叠上下文是元素在 z 轴上进行独立层叠排序的环境。一个层叠上下文内部的元素会先在内部比较层级，再作为整体参与父级层叠。

::: details 详情
### 为什么重要

很多 `z-index` 不生效的问题，本质上是元素处在不同层叠上下文中。

即使子元素设置了很大的 `z-index`，也可能无法盖过另一个层叠上下文中的元素。
...
如果没有形成可比较的层叠关系，单纯设置 `z-index` 可能没有效果。

### 排查思路

- 看元素是否处在不同层叠上下文。
- 检查父元素是否设置了 `transform`、`opacity` 等属性。
- 检查定位和 `z-index` 是否作用在正确元素上。
- 使用浏览器 DevTools 查看层级和样式来源。

### 注意事项

- 不要随意给大容器加 `transform`，可能影响固定定位和层叠。
- 弹窗、下拉菜单、tooltip 通常需要统一管理层级。
- 大型项目建议定义统一 z-index 令牌，避免互相压制。
:::

## 41、BEM 命名规范是什么
BEM 是一种 CSS 命名方法，分别表示 Block、Element、Modifier。它通过约定类名结构降低样式冲突和维护成本。
...
CSS Modules 是构建能力，会把类名局部化。

两者可以一起使用，但使用 CSS Modules 后，对全局唯一命名的依赖会降低。

### 注意事项

- 不要写过深的元素层级，例如 `block__a__b__c`。
