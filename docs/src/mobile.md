...
## 25、移动端弱网场景如何优化用户体验
移动端弱网优化要同时处理加载慢、请求失败、网络切换和用户感知问题。核心不是简单重试，而是让页面在网络不稳定时仍然可理解、可恢复。
...
## 26、移动端推送通知要注意哪些问题
移动端推送通知涉及系统权限、厂商通道、消息触达、用户体验和合规。设计时要保证重要消息可达，同时避免过度打扰用户。
...
1. 业务系统生成消息。
2. 推送服务做消息过滤和路由。
3. 调用 APNs、FCM 或国内厂商通道。
4. 系统通知中心展示消息。
5. 用户点击后唤起 App 或跳转页面。

如果 App 在线，也可以通过长连接或应用内消息补充。
...
- token 和用户绑定关系要可更新。
- 要统计发送量、到达率、点击率和关闭率。
- iOS 和 Android 权限模型差异较大，要分别处理。
:::

## 27、移动端崩溃监控和符号化是什么
移动端崩溃监控用于收集 App 崩溃现场，符号化用于把崩溃堆栈中的地址还原成可读的函数名、文件名和行号。没有符号化的崩溃日志通常很难定位问题。
...
崩溃堆栈里可能只有内存地址或混淆后的方法名。

符号化需要上传对应版本的符号文件，例如：

- iOS 的 dSYM。
- Android 的 mapping 文件。
- 原生库的符号文件。
...
## 28、移动端热修复要注意什么
移动端热修复用于在不重新发版的情况下修复线上问题。它能缩短故障恢复时间，但也会带来稳定性、合规和版本管理风险。
...
## 30、移动端包体积如何优化
移动端包体积优化可以降低下载成本、安装成本和启动加载压力。核心是分析体积来源，删除无用资源，并按需加载非核心能力。
...
如果把太多资源改成运行时下载，可能导致首屏变慢或弱网体验变差。

需要在安装包大小、启动速度、运行时体验之间平衡。
...
## 31、移动端后台任务为什么会受限制
移动端系统会限制后台任务，主要是为了控制耗电、流量和系统资源。应用不能假设进入后台后仍能长期执行任意逻辑。
...
这些任务通常需要使用系统提供的后台能力，而不是普通 JS 定时器。
