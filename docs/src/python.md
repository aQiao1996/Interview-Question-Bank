...
## 52、Python 项目如何管理依赖
Python 项目依赖管理要解决依赖声明、版本锁定、虚拟环境隔离和部署可复现问题。常见工具包括 pip、venv、pip-tools、Poetry 和 uv。
...
- `requirements.txt`：常见于 pip 项目。
- `pyproject.toml`：现代 Python 项目配置入口。
- lockfile：锁定完整依赖树版本。

应用项目通常需要 lockfile，库项目通常要更关注版本范围兼容。
