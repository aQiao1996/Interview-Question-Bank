...
## 35、vue3 中 v-pre 有什么作用
`v-pre` 用于跳过当前元素及其子元素的编译，让其中的模板语法按原始文本展示，也可以减少不需要编译区域的编译成本。
...
页面会直接显示 `{{ message }}`，而不是把它当作 Vue 插值表达式执行。

### 适合场景

- 展示代码示例。
- 展示模板语法文档。
- 页面中有一段静态内容不需要 Vue 编译。
- 避免第三方模板语法和 Vue 插值语法冲突。

### 和 v-once 的区别

- `v-pre` 是跳过编译，里面的 Vue 语法不会生效。
- `v-once` 会正常编译和渲染一次，后续响应式变化不再更新。
...
第一行显示原始文本，第二行显示首次渲染时的 `count` 值。

### 注意事项

- `v-pre` 内部的指令、插值和组件都不会被编译。
